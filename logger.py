...
def logger(
    log_file: str = f"./parafac-log/training-{time}.log",
...
class TrainTestLog:
    ...
    def create_csv(self):
        ...
        self.score_log_df.to_csv(
            f"./parafac-log/training-log-top{self.k}-{time}.csv", index=False
        )
        self.output_recs_df.to_csv(
            f"./parafac-log/Top-{self.k} Recommendation on testData - {time}.csv",
            index=False,
        )
