...
def get_recommendations(
    user_id, time_id, le_user: LabelEncoder, le_time: LabelEncoder, factorized_tensor, k
) -> np.ndarray:
    ...
    top_items = np.argsort(user_predictions)[-k:][
        ::-1
    ]  # Get K-latest values and reverse output from highest to lowest

    return top_items


def convert_result_to_org_format(
    ...
    for user_id in df["user"].unique():
        ...
        for time_id in users["timestamp"].unique():
            ...
            df.loc[(df["user"] == user_id) & (df["timestamp"] == time_id), "item"] = (
                le_item.inverse_transform(top_items).item()
            )
            df.loc[(df["user"] == user_id) & (df["timestamp"] == time_id), "rate"] = (
                user_predictions[top_items].item()
            )
...
def eval_flatten_calc(y_true: np.array, y_pred: np.array) -> dict:
    ...
    assert np.all(y_true >= 0) and np.all(
        y_pred >= 0
    ), "There is negative values in y_true or y_pred"
...
def calculate_map(
    test_df: pd.DataFrame, user_recs: dict, le_item: LabelEncoder, k: int = 5
) -> float:
    ap_sum = 0
    num_users = 0
    for user in test_df["user"].unique():
        ...
        recommended_items = [
            le_item.inverse_transform(np.array([i])).item() for i in user_recs[user]
        ]
        if not actual_items:
            continue
        ap = 0
...
def calculate_f1_score(
    ...
    for user in test_df["user"].unique():
        ...
        precision = len(actual_items.intersection(recommended_items)) / len(
            recommended_items
        )
        recall = len(actual_items.intersection(recommended_items)) / len(actual_items)
