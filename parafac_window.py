...
from metrics import eval_flatten_calc, get_recommendations, user_item_history
        ...
        for init_kernel in INIT_KERNEL:
            for n_components in N_COMPONENTS:
                for n_iter in N_ITER:
                    logger.info(
                        f"Training for window_size: {window} | range (day): {start_idx} to {end_idx} | init: {init_kernel} | n_components: {n_components} | n_iter: {n_iter} | tolerance: {TOLERANCE}"
                    )
                    start = timer()
                    try:
                        ...
                        logger.error(
                            f"Failed for window_size: {window} | range (day): {start_idx} to {end_idx} | init: {init_kernel} | n_components: {n_components} | n_iter: {n_iter}"
                        )
                        continue
                    stop = timer()
                    ...
                    metrics = eval_flatten_calc(
                        y_true=np.array(actual_values), y_pred=np.array(predictions)
                    )
                    logger.info(
